...
public class SearchOrderbyOrderNumber {
	WebDriver driver;
	WebDriverWait wait;
...
@DataProvider (name="SearchOrderNumberData")
	public Object[][] orderDataProviderMethod() {
	    Object[][] retObjArr=getDataFromXLSUsingJXL("test\\resources\\data\\SearchOrderNumber.xls",
	            "OrderNum", "StartAndEnd");
	      return(retObjArr);	  
	}
@Test (dataProvider = "SearchOrderNumberData")
  	public void SearchbyOrderNumber(String OrderNum) throws InterruptedException, IOException {
		...
		output = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("menu-orders")));
		  ...
		  output = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("menu-orders-search")));
		  WebElement searchOrderLink= driver.findElement(By.id("menu-orders-search"));
		  searchOrderLink.click(); //Search order from Left navigation
	
		  searchOrder(OrderNum); //Search Order method
	
}

	private void searchOrder(String OrderNum) throws InterruptedException {
	...
	// TODO Auto-generated method stub
		...
		try {
			...
			output = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@name='dfOrderNumber' and @class='form-control']"))); 
			WebElement orderNumberEntry = driver.findElement(By.xpath("//input[@name='dfOrderNumber' and @class='form-control']"));
			
			orderNumberEntry.clear();
			orderNumberEntry.sendKeys(OrderNum);
