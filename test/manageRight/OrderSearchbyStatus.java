...
public class OrderSearchbyStatus {
	WebDriver driver;
	WebDriverWait wait;
...
@DataProvider (name="SearchOrderStatusData")
	public Object[][] orderDataProviderMethod() {
	    Object[][] retObjArr=getDataFromXLSUsingJXL("test\\resources\\data\\SearchOrderStatus.xls",
	            "OrderNum", "StartAndEnd");
	      return(retObjArr);	  
	}
@Test ()
  	public void OrderSearchStatus() throws InterruptedException, IOException {
		...
		output = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[contains(text(),'Orders')]")));
		  ...
		  searchOrder(); //Search Order method
	
}

	private void searchOrder() throws InterruptedException {
	...
	// TODO Auto-generated method stub
		...
		try {
			...
			output = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//select[@name='cmbOrderStatus']")));
			Select orderStatus = new Select(driver.findElement(By.xpath("//select[@name='cmbOrderStatus']")));
			
			orderStatus.selectByValue("5"); //as per data excel
